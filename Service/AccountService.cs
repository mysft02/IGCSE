...
namespace Service
{
    public class AccountService
    {
        ...
        public async Task<BaseResponse<object>> CreateParentAccountAsync()
        {
            try
            {
                // Kiểm tra xem tài khoản parent đã tồn tại chưa
                var existingParent = await _userManager.FindByNameAsync("parent");
                
                if (existingParent == null)
                {
                    // Tạo tài khoản parent mới
                    ...
                    var result = await _userManager.CreateAsync(parentAccount, "A123456789a!");
                    if (result.Succeeded)
                    {
                        // Gán role Parent
                        await _userManager.AddToRoleAsync(parentAccount, "Parent");
                        
                        var token = await _tokenRepository.createToken(parentAccount);
                        
                        return new BaseResponse<object>(
                            "Tạo tài khoản Parent thành công",
                            StatusCodeEnum.OK_200,
                            new
                            {
                                Message = "Tài khoản Parent đã được tạo",
                                Username = "parent",
                                Email = "parent@gmail.com",
                    ...
                    {
                        throw new Exception($"Không thể tạo tài khoản Parent: {string.Join(", ", result.Errors.Select(e => e.Description))}");
                    }
                }
                else
                {
                    // Tài khoản đã tồn tại, kiểm tra và cập nhật role
                    var currentRoles = await _userManager.GetRolesAsync(existingParent);
                    if (!currentRoles.Contains("Parent"))
                    {
                        await _userManager.AddToRoleAsync(existingParent, "Parent");
                    }

                    var token = await _tokenRepository.createToken(existingParent);
                    
                    return new BaseResponse<object>(
                        "Tài khoản Parent đã tồn tại và được cập nhật role",
                        StatusCodeEnum.OK_200,
                        new
                        {
                            Message = "Tài khoản Parent đã sẵn sàng",
                            Username = "parent",
                            Email = "parent@gmail.com",
                            Password = "A123456789a!",
                            Token = token.AccessToken,
            ...
            {
                throw new Exception($"Failed to create parent account: {ex.Message}");
            }
        }
    }
}
